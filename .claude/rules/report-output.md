...
报告写入前，先读取 [模板目录说明](../../templates/README.md) 和匹配的模板；不要凭记忆另造目录。
...
| 行业研究 | `industry-research` | `templates/industry-research.md` | `reports/{行业名}-industry-{YYYYMMDD}.md` |
...
- 已有旧报告不移动、不重命名。读取既有个股报告、论文或系列时，先查本表的新位置；未找到时才以只读方式兼容旧的 `reports/{公司名}/` 与 `reports/{公司名}-*.md` 路径。
- 只有用户明确要求迁移时，才批量处理旧路径，并先列出受影响文件和目标路径供确认。
