...
- `.claude/skills/*/SKILL.md` 是唯一 workflow 正文源；Codex 的 `.agents/skills/*/SKILL.md` 只允许由 `uv run python scripts/sync-codex-skill-adapters.py` 生成的薄适配层，不得手工复制或改写完整正文。
...
- 新增或修改 skill 后，使用 `uv run python scripts/sync-codex-skill-adapters.py` 同步；删除或改名 skill 时加 `--prune` 清理本脚本生成的旧 wrapper，再用 `--check` 检查是否过期；不要直接调用系统 `python3`。
