---
description: "AI Berkshire 项目内工作流：供应链瓶颈猎手：AI驱动的全球产业链瓶颈套利"
...
**台股标的取数**：供应链扫描常命中台股（4位数字代码，如 2330、3661），其行情/PER/市值/月营收按 [.claude/skills/financial-data/SKILL.md 台股单链](../../skills/financial-data/SKILL.md) 处理（FinMind 主源，`tools/data_loader.py` 统一登记）。台股每月10日前强制披露上月营收，月营收同比是验证瓶颈"量价齐升"最快的公开信号。
...
增量运行前，按报告路由规则读取已有总地图与观察名单；首次运行则建立基线。状态文件的输出位置、命名与读者可见结构均以 [.claude/rules/report-output.md](../../rules/report-output.md) 及其指定模板为准。
...
有明确标的、仅信号扫描、完整扫描与状态文件的最终呈现均以 [.claude/rules/report-output.md](../../rules/report-output.md) 及其指定模板为准。本 Skill 不定义文件命名、目录或最终报告版式。
...
9. **遵循 `.claude/rules/research-quality.md`** — 不预设看多，先数据后结论

---

## 输出要求

1. 生成前读取 [.claude/rules/report-output.md](../../rules/report-output.md) 及其指定模板；模板是最终报告、快报、总地图和观察名单的唯一呈现定义。
2. **语言**：中文
3. **风格**：直接、犀利、不说废话
