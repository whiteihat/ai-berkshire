---
description: "AI Berkshire 项目内工作流：财报精读团队：四大师并行解读 + 公众号发布"
...
> **Codex 适配说明**：有并行子代理能力时按本 workflow 并行执行；当前 Codex 不提供并行子代理或后台任务时，按角色和阶段顺序执行，不因无法并行而伪造并行结果，并在报告或过程说明中标注该降级。
...
读者评审稿的读者可见结构以指定模板为准；本 Agent 只按上述可读性、信息价值、可信度与行动指导性维度审阅。
...
生成前读取 [.claude/rules/report-output.md](../../rules/report-output.md) 及其指定模板。读者稿、研究底稿、视角稿和评审稿的最终结构及文件产物以模板和路由规则为准；本 Skill 只保留角色分工、合成、编辑、读者评审和准出流程。

对路由规则指定的最终报告执行 `tools/report_audit.py extract` 与 `verdict`。**【准出】** 全部通过 → 可发布；**【打回】** 有不通过 → 修正后重审。
