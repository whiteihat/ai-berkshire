---
description: "AI Berkshire 项目内工作流：未上市公司研究：多Agent并行深度研究框架"
...
> **Codex 适配说明**：有并行子代理能力时按本 workflow 并行执行；当前 Codex 不提供并行子代理或后台任务时，按六个角色顺序执行，不因无法并行而伪造并行结果，并在报告或过程说明中标注该降级。
...
综合 6 份分析报告，生成前读取 [.claude/rules/report-output.md](../../rules/report-output.md) 及其指定模板。最终报告的章节、评分表、数据拼图、信号一致性矩阵、估值判断、风险矩阵、退出路径、决策表与跟踪清单严格以模板为准；本 Skill 只保留六位分析师的分工、信息拼图与反偏见检查流程。

### 第八步：保存报告

按 [.claude/rules/report-output.md](../../rules/report-output.md) 路由规则写入；不得覆盖已有报告。
