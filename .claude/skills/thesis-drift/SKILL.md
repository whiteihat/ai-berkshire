---
description: "AI Berkshire 项目内工作流：投资论文漂移检测：分清事实变化与措辞变化"
...
- `公司名 reports/01-单公司分析/{公司名}/{公司名}-thesis-旧日期.md reports/01-单公司分析/{公司名}/{公司名}-thesis-新日期.md` — 对比两份新规范路径下的论文快照
- `公司名` — 优先查找 `reports/01-单公司分析/{公司名}/{公司名}-thesis.md` 及同目录历史快照；找不到时只读兼容旧路径 `reports/{公司名}-thesis.md` 和 `reports/{公司名}/`
...
- 如果只提供公司名 → 优先查找 `reports/01-单公司分析/{公司名}/{公司名}-thesis.md` 及历史快照；找不到时只读兼容旧路径，再进入**自动快照对比**模式
...
```bash
uv run python tools/financial_rigor.py verify-market-cap --price {价格} --shares {股本} --reported {报告市值} --currency {币种}
uv run python tools/financial_rigor.py cross-validate --field {字段} --values '{JSON}' --unit {单位}
uv run python tools/financial_rigor.py three-scenario --price {价格} --eps {EPS} --shares {股本亿} --growth {乐观} {中性} {悲观} --pe {乐观PE} {中性PE} {悲观PE}
uv run python tools/financial_rigor.py calc --expr '{精确算式}'
```
...
生成前读取 [.claude/rules/report-output.md](../../rules/report-output.md) 及其指定模板。最终报告的章节、维度表、结论呈现和占位符严格以模板为准；本 Skill 只保留证据归一化、数值验算、三态漂移判定与错误配对防护规则。

按报告路由规则写入，且不得覆盖论文基线；若同日文件已存在，创建草稿/版本文件或先询问。
...
按以下顺序查找：
- `reports/01-单公司分析/{公司名}/{公司名}-thesis.md`
- `reports/01-单公司分析/{公司名}/{公司名}-thesis-drift-*.md`
- `reports/{公司名}-thesis.md`（旧路径，只读兼容）
- `reports/{公司名}/` 及 `reports/01-单公司分析/{公司名}/` 中包含 `thesis`、`论文`、`追踪` 的报告
...
4. 如果当前报告已足够完整，可建议将它保存为 `reports/01-单公司分析/{公司名}/{公司名}-thesis.md` 作为未来漂移检测基线
