...
检查是否已存在该公司的投资论文文件（`reports/01-单公司分析/{公司名}/{公司名}-thesis.md`）；为兼容历史成果，再只读检查旧路径 `reports/{公司名}-thesis.md`：
- 新路径或旧兼容路径均不存在 → 进入**建立论文**模式
- 找到新路径 → 进入**追踪检查**模式
- 只找到旧路径 → 先提示将按旧文件只读兼容，询问是否迁移后再更新；未经确认不移动文件
- 如果找不到但用户表示已有 → 询问文件路径
...
读取 `reports/01-单公司分析/{公司名}/{公司名}-thesis.md`，加载：
- 核心论文
- 核心假设清单
- 红线清单
- 上次检查记录
...
将本次检查记录追加到 `reports/01-单公司分析/{公司名}/{公司名}-thesis.md` 的追踪记录表中：
