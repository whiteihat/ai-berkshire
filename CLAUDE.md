...
- 决策敏感财务数据按 [财务数据规范](.claude/skills/financial-data/SKILL.md) 取主源并做合理性校验（不做强制双源比对，异常才人工核查）；金额必须标明币种和单位；市值、估值、比例等计算使用 `tools/financial_rigor.py`，不得以心算替代。
...
面向使用者的安装、运行、工具与排障说明见 [运行指南](docs/运行指南.md)。数据源优先级、主源取数与合理性校验、退化路径以 [财务数据规范](.claude/skills/financial-data/SKILL.md) 为准。
