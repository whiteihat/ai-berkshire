...
AI Berkshire 是一套由 Claude Code 执行的价值投资研究 Skill 工作流。`skills/` 是工作流源文件，`templates/` 定义标准报告结构，`reports/` 存放研究产出。

## 不可省略的规则

- 研究报告使用中文。将**事实**、**观点/分析判断**、**估计/假设**与**数据不足**明确区分；不得把推测写成事实。
- 重大结论应给出证据来源、最强反证与会改变结论的条件。数据不足时如实说明，不以套话填充确定性。
- 决策敏感财务数据须按 [财务数据规范](skills/financial-data.md) 交叉验证；金额必须标明币种和单位；市值、估值、比例等计算使用 `tools/financial_rigor.py`，不得以心算替代。
...
面向使用者的安装、运行、工具与排障说明见 [运行指南](docs/运行指南.md)。数据源优先级、口径、误差阈值与退化路径以 [财务数据规范](skills/financial-data.md) 为准。
