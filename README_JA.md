...
すべての計算はPython `decimal.Decimal`（厳密な10進算術）を使用し、`float`は使いません。データは統一アクセス層を通じて主ソースを取得してローカルに永続化し、合理性チェック（強制の二重ソース比較はしない、異常時のみ人手で再確認）を行います。
...
| Skill | 目的 | 使用場面 |
|-------|------|---------|
| [`/stock-pipeline`](.claude/skills/stock-pipeline/SKILL.md) | 単一銘柄 標準パイプライン（オーケストレーター） | 選別から買入後追跡まで——推奨エントリ |
| [`/fund-pipeline`](.claude/skills/fund-pipeline/SKILL.md) | 単一ファンド 標準パイプライン（オーケストレーター） | アクティブ/パッシブ分岐から買入後追跡——推奨エントリ |
...
| Skill | 目的 | 使用場面 |
|-------|------|---------|
| [`/financial-data`](.claude/skills/financial-data/SKILL.md) | 財務データの取得と合理性チェック | 統一data_loaderが主ソースを取得してローカルに永続化；異常時のみ人手で再確認 |
...
```bash
# 標準ワークフロー（推奨エントリポイント）
/stock-pipeline テンセント   # 単一銘柄：選別→深掘り→関門→追跡
/fund-pipeline 510300        # 単一ファンド：アクティブ/パッシブ分岐→研究→追跡
...
- 重要データは統一データ層を経由して合理性チェック（時価総額手動計算、異常時のみ再確認）
...
| **合理性チェック** | `cross-validate` | 主ソースデータと前値・常識が逸脱した場合に人手で比較；許容範囲超でアラート |
