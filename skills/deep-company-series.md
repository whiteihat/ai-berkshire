...
参考样本：`reports/01-单公司分析/腾讯/《看懂腾讯》/`（旧版目录仅作参考，不覆盖历史成果）
...
- 每篇写完先存 `reports/01-单公司分析/{公司名}/《看懂{公司名}》-{YYYYMMDD}/0X-XX.md`
- **目录冲突规则**：若目标日期目录已存在，不覆盖已有文件；旧版 `reports/{公司名}/《看懂{公司名}》/` 只读兼容，不移动、不混放。
- 不立即推送——等待用户审阅
- 用户提修订意见后修改
- 只有用户明确要求后，才按当前工作树和实际 remote 执行 Git 操作
...
```bash
# 用户明确要求发布前，先扫描报告中的常见本机隐私标识
grep -r "<本机用户名>\|/Users/\|<个人身份信息>" reports/ | head
```

确认无误后，若用户明确要求发布，再先检查 `git status --short` 与 `git remote -v`，按实际仓库状态决定后续 Git 操作。
