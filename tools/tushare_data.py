...
token 读取（token 只存本机，严禁提交到 git；local/ 已被 .gitignore 永久排除）：
    ...
    cheapyun 代理（cheapyun_token）：①环境变量 CHEAPYUN_TOKEN  ②本地文件 local/tushare_token_cheapyun.txt
...
class _Client:
    ...
    def __init__(self):
        ...
        cheapyun_token = _read_token("CHEAPYUN_TOKEN", "tushare_token_cheapyun.txt")
        official_mod = self._try_import("tushare")
        if official_mod and cheapyun_token:
            self._sources.append(
                (official_mod, "cheapyun代理", cheapyun_token,
                 "cheapyun 代理：环境变量 CHEAPYUN_TOKEN 或 local/tushare_token_cheapyun.txt",
...
def cmd_check():
    ...
    source_defs = [
        ...
        ("cheapyun代理", "CHEAPYUN_TOKEN", "tushare_token_cheapyun.txt", "tushare"),
